---
title: "Workshop 4"
subtitle: " More portfolio analytics"
output:
  learnr::tutorial:
  css: ['mycssblend.css']
...
> Before you begin, run the following to update the `ati` package
...
library(tidyquant)
library(ati)
library(timetk) # this may be a need package you will need to install
...
## Topic 3 Detoning
De-noised correlation matrix from the previous methods can also be de-toned by excluding a number of first eigenvectors representing the market component. According to Prof. Marcos Lopez de Prado
...
```{r comparemore}
model1<-estRMT(ftse_r_m[,-1],detone = TRUE)
...
## Topic 4 portfolio evaluation of denoising
...
Set up and run a backtest strategy to test the difference in using a denoised covariance matrix for for portfolio optimisation
...
```{r rmt-backtesting}
registerDoSEQ() # explicitly register a sequential parallelising backend
opt.ordinary <- optimize.portfolio.rebalancing(
  ...
  trailing_periods=30)
opt.rmt <- 
  optimize.portfolio.rebalancing(ftse_r_m_ts, pspec.lo, 
...
### Build portfolio returns and visualise backtesting results
